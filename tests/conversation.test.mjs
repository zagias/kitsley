...
test('AI adapter sends bounded context, hides credentials and enforces persistent preview budget',async()=>{
 ...
 let calls=0;globalThis.fetch=async(url,options)=>{calls++;assert.equal(url,'https://api.openai.com/v1/responses');const body=JSON.parse(options.body);assert.equal(body.store,false);assert.equal(body.max_output_tokens,700);return Response.json({status:'completed',output:[{content:[{type:'output_text',text:'What will you store?'}]}]});};
 ...
 try{assert.deepEqual(await (await POST(req())).json(),{text:'What will you store?'});assert.equal((await POST(req())).status,429);assert.equal(calls,1);}finally{globalThis.fetch=originalFetch;for(const k of keys){if(old[k]===undefined)delete process.env[k];else process.env[k]=old[k];}await rm(dir,{recursive:true,force:true});}
});
