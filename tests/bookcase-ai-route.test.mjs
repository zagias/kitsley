...
test('Bookcase requests use the current guide, skip discovery and fail closed for invalid dimensions',async t=>{
 ...
 let sent,calls=0;globalThis.fetch=async(url,init)=>{calls++;assert.equal(url,'https://api.openai.com/v1/responses');sent=JSON.parse(init.body);return Response.json({status:'completed',output:[{content:[{type:'output_text',text:JSON.stringify({text:'Step 2: use 180 grit on the faces.',steps:[{step:2,guidance:'Use 180 grit on the faces.'}]})}]}]});};
