...
export async function ownedProject(db,userId,projectId){
 ...
 return data?.entities?.['conversation:'+projectId]||null;
}
...
export async function reserveUsage(projectId,kind){
 ...
 try{
  const db=billingDB(),project=await ownedProject(db,user.id,projectId);if(!project)return {response:billingReply({error:'Save this project to your account before continuing.',code:'sync_required'},409)};
  ...
  return {db,user,project,id:data.id,plan:data.plan};
