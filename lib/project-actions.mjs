...
export function proposeChange(record,text){
 ...
 if(/\b(mdf|oak|pine|solid wood|glass|metal)\b/i.test(text)&&/use|change|make|instead|switch/i.test(text))return {unsupported:'This drawing supports plywood only. I saved your request, but have not substituted another structural material.'};
 ...
 const pattern=/(\d+(?:\.\d+)?)\s*(mm|cm|m|inches|inch|in|")?\s*(wide|width|tall|high|height|deep|depth|thick|thickness)\b/gi;
 ...
 for(const m of text.matchAll(/\b(width|height|depth|thickness)\s*(?:to|of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(mm|cm|m|inches|inch|in|")?/gi))patch[m[1].toLowerCase()]=millimetres(m[2],m[3]||'mm');
 ...
 for(const key of Object.keys(patch))if(Number(current[key])!==next[key])changes.push({key,before:Number(current[key]),after:next[key]});
 if(!changes.length)return {unchanged:true};
 return {input:Object.fromEntries(['width','height','depth','thickness','shelves'].map(k=>[k,next[k]])),changes};
}
