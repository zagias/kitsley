...
export function urgentIntent(text){return /sewage|gas\s*(smell|leak)|smell.*gas|hissing.*gas|sparking|burning smell|smoke|flood|burst pipe|water.*(outlet|electri|ceiling)/i.test(text);}
export function matchProject(text){
 ...
 const matches=searchLibrary({query:text});return matches.find(p=>p.mode!=='urgent'&&p.category!=='safety')?.id||null;
}
