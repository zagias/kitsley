import {advicePurposeInstructions} from './advice-purpose.mjs';
import {scopeInstructions} from './diy-scope.mjs';
...
Before returning, check that every factual citation in text also appears in research.urls and was actually retrieved. Do not use a secondary article for a claim while listing an unrelated primary source as its evidence. Check that the source actually states each threshold: a recommendation to use the highest filter a system accommodates is not a rule that compatibility checks start only above MERV 13. Absence of an alarm, visible moisture or detector signal is not proof that a hazard is absent. If exact equipment or site facts are missing, distinguish supported general principles from unverified suitability; mark that suitability needs-details. Keep the useful supported explanation rather than inventing a procedure. Return a research object: status is 'supported' only when the answer is within the documented scope and required facts are known; 'needs-details' when essential facts are missing; 'conflicting' for unresolved evidence; 'limited' for insufficient support or unsupported designs. note is one short explanation. urls contains only the exact primary-source URLs actually retrieved AND used for your answer (not every search hit). If you searched, cite relevant statements in text using [source title](exact URL), also listing those URLs in research.urls. Do not invent source URLs. A URL supplied by the user or a previous answer is not evidence that it was retrieved. If you cannot retrieve the requested manual, say that plainly; link only relevant primary pages actually returned, and do not transfer specifications from a different model. Keep step guidance plain text; its source links are stored separately. Keep citations out of numeric geometry. For needs-details, conflicting or limited, give only what is established and the next useful check/question; leave steps empty. Sources inform personal guidance; they do not certify it.`+advicePurposeInstructions+scopeInstructions+assessmentInstructions+designOperationInstructions+specialistInstructions;
...
export function researchFormat(base,{assessmentRequired=false}={}){
 ...
 schema.properties.scope={type:'string',enum:['in-scope','uncertain','out-of-scope']};
 schema.required.push('scope','research','technicalAssessment','designOperation','specialistReview');
...
// Known aggregators/code mirrors are discovery leads, not authoritative evidence.
// This exclusion does not certify every other host as a primary source.
const nonPrimaryHosts=['buildingcode.online','reddit.com','wikipedia.org','pinterest.com'];
const eligibleEvidenceUrl=value=>{try{const h=new URL(value).hostname;return !nonPrimaryHosts.some(x=>h===x||h.endsWith('.'+x));}catch{return false;}};
...
export function researchEvidence(data,raw,{required=false,now=new Date().toISOString()}={}){
 ...
 const sources=[...new Set((Array.isArray(raw?.urls)?raw.urls:[]).map(sourceUrl).filter(Boolean))].map(url=>returned.get(url)).filter(s=>s&&eligibleEvidenceUrl(s.url)).slice(0,8);
...
// A retrieved URL is provenance, not proof of truth. This guard catches a narrower
// mechanical failure: prose citing evidence that the response did not establish.
export function answerEvidenceIssues(text,research){
 const allowed=new Set((research?.sources||[]).map(s=>sourceUrl(s.url)));
 const urls=[...String(text||'').matchAll(/https?:\/\/[^\s)<>]+/g)].map(m=>sourceUrl(m[0].replace(/[.,;]+$/,''))).filter(Boolean);
 const issues=[];
 if(urls.some(u=>!allowed.has(u)))issues.push('unverified-citation');
 if(research?.searched&&allowed.size===0)issues.push('no-retrieved-evidence');
 return issues;
}
