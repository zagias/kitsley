import {workshopHTML} from './bookcase-workshop-export.mjs';
import {workshopModel,workshopShopping} from './bookcase-workshop.mjs';
...
import {contentFor} from './project-content.mjs';
import {manualHTML} from './guide-manuals.mjs';
...
export function shoppingRows(record,items,owned,stock=[]){
 if(record.guideId==='bookcase'&&record.pack?.build?.version===1)return workshopShopping(workshopModel(record.pack.input),record.pack.build.options,owned,stock);
...
export function exportProject(record,items,owned,stock=[]){
 if(record.guideId==='bookcase'&&record.pack?.build?.version===1)return workshopHTML(record.pack.input,record.pack.build.options,owned,stock);
