import {advicePurpose} from './advice-purpose.mjs';
import {scopeReply} from './diy-scope.mjs';
import {diagnosticGuidance,diagnosticGuidanceVersion} from './diagnostic-guidance.mjs';
...
import {researchVersion,answerEvidenceIssues} from './advice-research.mjs';
...
export function engineDecision({record={},question='',raw={},result,research}){
 if(raw.scope==='out-of-scope')return scopeReply();
 ...
 const diagnostic=diagnosticGuidance(question);
 const evidenceIssues=answerEvidenceIssues(result.text,research);
 if(evidenceIssues.length)research={...research,status:'limited',note:'The answer contained claims without verified supporting citations.'};
 ...
 const geometryOnly=!evidenceIssues.length&&!assessment&&['supported','limited','needs-details'].includes(research.status)&&raw.designOperation?.factsComplete===true&&raw.designOperation?.builder==='bookcase.recessed-shelves.v1'&&raw.designOperation?.target==='interior-shelves';
 ...
 if(evidenceIssues.length){
  presentation={...result,text:'I could not confirm the sources for a reliable answer to this question. Your project is unchanged. Please retry so Kitsley can check the evidence again.',stepUpdates:[]};
  delete presentation.discovery;
 }
 if(diagnostic&&(evidenceIssues.length||assessment?.stage==='needs-checking')){
  if(assessment)assessment.question=diagnostic.question;
  const advice=diagnostic.text+' ['+diagnostic.source.title+']('+diagnostic.source.url+')\n\n'+diagnostic.question;
  presentation={...presentation,text:advice,...(presentation.discovery?{discovery:{...presentation.discovery,advice,question:diagnostic.question,choices:[],briefReady:false}}:{})};
  research={...research,sources:[...research.sources.filter(s=>s.url!==diagnostic.source.url),diagnostic.source]};
 }
 ...
 if(assessment?.stage==='needs-checking'&&!diagnostic){
  ...
  const supported=assessment.establishedPrinciples.length?assessment.establishedPrinciples:established.map(c=>({text:c.finding,references:c.references}));
  const facts=supported.map(c=>c.text+' ['+c.references[0].title+']('+c.references[0].url+')').join('\n\n');
