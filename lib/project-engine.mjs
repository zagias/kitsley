...
export function engineDecision({record={},question='',raw={},result,research}){
 ...
 if(assessment?.stage==='needs-checking'){
  ...
  const nextQuestion=diagnostic?.question||assessment.question;
  const advice=diagnostic?diagnostic.text+' ['+diagnostic.source.title+']('+diagnostic.source.url+')\n\n'+nextQuestion:(facts?facts+'\n\n':'')+'Before choosing the method: '+nextQuestion;
  if(diagnostic)assessment.question=nextQuestion;
