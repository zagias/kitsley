...
export function compileProposal(record,operation,question=''){
 ...
 if(operation.target==='interior-shelves'&&/\b(narrow|narrower|width|wide|taper|angled|top shelf|top panel|dog|cage|wall.hung)\b/i.test(question))return {status:'unsupported',reason:'Clarify the axis and support arrangement before changing one shelf.'};
 try{
...
export function engineDecision({record={},question='',raw={},result,research}){
 ...
 const geometryOnly=!assessment&&['supported','limited'].includes(research.status)&&raw.designOperation?.builder==='bookcase.recessed-shelves.v1'&&raw.designOperation?.target==='interior-shelves';
 const proposal=research.status==='supported'||geometryOnly?compileProposal(record,raw.designOperation,question):null;
 ...
 if(assessment?.stage==='needs-checking'){
  const advice='Some material or method decisions still need evidence. Your current design is unchanged; check the open decisions below before choosing how to build it.';
