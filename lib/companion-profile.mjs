...
export function validEquipment(v){return !!v&&typeof v==='object'&&/^[a-zA-Z0-9_-]{1,80}$/.test(v.id)&&ownedChoices.includes(v.toolId)&&['make','model','variant'].every(k=>text(v[k],100))&&text(v.accessories,400)&&(v.make.trim()||v.model.trim())&&v.source==='user'&&text(v.updatedAt,40);}
...
export function cleanCompanion(equipment=[],experience=[],owned=[]){
 return {equipment:(Array.isArray(equipment)?equipment:[]).filter(v=>validEquipment(v)&&owned.includes(v.toolId)).slice(0,100).map(v=>({id:v.id,toolId:v.toolId,make:v.make,model:v.model,variant:v.variant,accessories:v.accessories,source:'user'})).sort((a,b)=>a.id.localeCompare(b.id)),experience:(Array.isArray(experience)?experience:[]).filter(validExperience).map(v=>({id:v.id,level:v.level,source:'user'})).sort((a,b)=>a.id.localeCompare(b.id))};
}
export function companionFromEntities(entities={}){const values=(type)=>Object.entries(entities).filter(([k,v])=>k.startsWith(type+':')&&v!==null).map(([,v])=>v);return cleanCompanion(values('equipment'),values('experience'),values('tool'));}
...
// Only explicit first-person statements produce suggestions. Completed steps,
// jargon, payments and success counts are not evidence of competence.
