...
function restoreContent(clean,raw){
 ...
 clean.messages=clean.messages.map((m,i)=>['ai','guide','saved-answer','foundation','logic'].includes(raw.messages[i]?.source)&&m.role==='assistant'?{...m,source:raw.messages[i].source,foundationIds:foundationKnowledge.filter(e=>raw.messages[i].foundationIds?.includes(e.id)).map(e=>e.id),research:savedResearch(raw.messages[i].research),technicalAssessment:restoreAssessment(raw.messages[i].technicalAssessment)}:m);
