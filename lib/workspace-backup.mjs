...
function restoreContent(clean,raw){
 ...
 const progress=raw.instructionProgress,steps=contentFor(clean.guideId)?.steps.length;
 if(steps&&progress?.key===contentProgressKey(clean))clean.instructionProgress={key:contentProgressKey(clean),active:Number.isInteger(progress.active)&&progress.active>=0&&progress.active<steps?progress.active:0,done:[...new Set(Array.isArray(progress.done)?progress.done.filter(i=>Number.isInteger(i)&&i>=0&&i<steps):[])],scopeAccepted:progress.scopeAccepted===true};
