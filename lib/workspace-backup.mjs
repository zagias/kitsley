import {cleanCompletion} from './project-learning.mjs';
...
import {restoreSpecialistReview} from './specialist-review.mjs';
...
function restoreContent(clean,raw){
 if(cleanCompletion(raw.completionReview))clean.completionReview=cleanCompletion(raw.completionReview);
 ...
 clean.messages=clean.messages.map((m,i)=>['ai','guide','saved-answer','foundation','logic','safety'].includes(raw.messages[i]?.source)&&m.role==='assistant'?{...m,source:raw.messages[i].source,foundationIds:foundationKnowledge.filter(e=>raw.messages[i].foundationIds?.includes(e.id)).map(e=>e.id),research:savedResearch(raw.messages[i].research),technicalAssessment:restoreAssessment(raw.messages[i].technicalAssessment),specialistReview:restoreSpecialistReview(raw.messages[i].specialistReview,savedResearch(raw.messages[i].research))}:m);
 if(raw.specialistReview)clean.specialistReview=restoreSpecialistReview(raw.specialistReview,null);
