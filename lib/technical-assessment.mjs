...
export function technicalAssessment(raw,research){
 ...
 for(const item of raw.checks){
  ...
  checks.push({topic:item.topic,finding:missingEvidence?'This decision still needs applicable source evidence before it can be used.':String(item.finding||'More information is needed.').slice(0,600),status,references});
 }
 ...
 return {version:1,summary:String(raw.summary||'Project decisions').slice(0,400),question:String(raw.question||'').slice(0,240),choices:(Array.isArray(raw.choices)?raw.choices:[]).filter(s=>typeof s==='string'&&s.trim()).slice(0,4).map(s=>s.slice(0,120)),checks,checkedAt:research?.checkedAt||null,stage:research?.status==='supported'&&checks.every(c=>['established','not-applicable'].includes(c.status))?'ready-for-planning':'needs-checking'};
}
