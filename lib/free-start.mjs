...
export function freeStartingPoint(record={}){
 ...
 if(blocked)return {title:'Understand the work before changing it',steps:[blocked.text],guide};
 const compatibility=methodCompatibility(request);if(compatibility)return {title:compatibility.title,steps:[compatibility.text],source:compatibility.source,guide:library.find(g=>g.id==='pex-line'&&compatibility.id==='pex-is-not-soldered')||guide};
