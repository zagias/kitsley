...
export function pathSetup(record){
 ...
 if(path?.id==='household'&&/\b(?:chang\w*|replac\w*)\b.{0,40}\b(?:hvac|furnace|air)\b.{0,20}\bfilter\w*\b/i.test(request))return {advice:'The filter change is already noted. Match the equipment and filter before buying a replacement.',questions:[{id:'constraint',text:'What filter size and equipment model can you read safely?',hint:'Use a visible label or your manual; leave unknown details open. Do not remove service panels to find it.'},experience]};
 if(path?.id==='household'&&/\b(?:mount\w*|install\w*|hang\w*)\b.{0,30}\b(?:tv|television)\b|\b(?:tv|television)\b.{0,20}\b(?:mount\w*|install\w*|hang\w*)\b/i.test(request))return {advice:'The TV mounting task is already noted. Match the wall and mounting system before drilling.',questions:[{id:'constraint',text:'What is the wall made of, and which TV and mount will you use?',hint:'Model numbers and the TV weight help check compatibility. “Unknown” is fine; do not drill to investigate.'},experience]};
 if(path?.id==='trade'){
  const named=[/\b(?:electrical|rewir\w*|wiring)\b/i,/\b(?:plumbing|pex)\b/i,/\bdrywall\b/i].filter(re=>re.test(request));
  if(named.length===1&&/\b(?:install\w*|replac\w*|repair\w*|patch\w*|fix\w*|rewir\w*|hang\w*)\b/i.test(request))return {advice:'Your type of work is already noted. The existing conditions and local requirements are the next details to check.',questions:[{id:'constraint',text:'Where will this work take place, and what condition is it in?',hint:'City or region and the existing material/system are enough; no street address is needed.'},experience]};
 }
