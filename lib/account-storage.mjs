import {collections,flatten,diff,equal,mergeChanges,activeEntities} from './workspace-sync.mjs';
...
export const workspaceStorage={
 ...
 setItem(key,value){if(scope===null)throw Error('Workspace is still loading.');if(scope==='guest'){localStorage.setItem(key,value);return;}const c=readCache();if(c.values[key]===value)return;c.values[key]=value;cacheWrite(c);report('pending','Saving to your account…');schedule();},
 removeItem(key){if(scope==='guest'){localStorage.removeItem(key);return;}const c=readCache();delete c.values[key];cacheWrite(c);report('pending','Saving to your account…');schedule();},
...
export async function syncNow(){
 ...
 running=(async()=>{try{
  ...
  for(let attempt=0;attempt<4;attempt++){
   ...
   let saved=remote;
   if(!equal(merged,remote.entities)){saved=await request('PUT',{userId:scope,revision:remote.revision,entities:merged});if(saved.conflict)continue;}
