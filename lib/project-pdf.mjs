import {displayLength as length,unitQuantity,measurementText,cutMeasurementNote} from './units.mjs';
...
const text=s=>String(s??'').replace(/[\u2010-\u2014]/g,'-').replaceAll('→','>').replaceAll('✓','Reviewed: ').replaceAll('↗','').replaceAll('↓','').replaceAll('≈','~');
...
export async function projectPDF(payload){
 ...
 const exactCell=n=>units==='imperial'?measure(n)+'\n('+Number(n.toFixed(6))+' mm)':measure(n);
 ...
 function art(svg,height=300){room(height+10);SVGtoPDF(doc,svg.replaceAll('≈','~'),42,y,{width,height,preserveAspectRatio:'xMidYMid meet',fontCallback:()=> 'Helvetica'});y+=height+12;}
 function table(headers,rows,cols,{padding=8,size=9}={}){
  function row(values,isHead=false){doc.font(isHead?'Helvetica-Bold':'Helvetica').fontSize(size);let h=Math.max(...values.map((v,i)=>doc.heightOfString(display(v),{width:cols[i]-padding*2,lineGap:2})))+padding*2+2;if(y+h>bottom){page();row(headers,true);}if(isHead)doc.rect(42,y,width,h).fill('#f1e9fc');let x=42;values.forEach((v,i)=>{doc.fillColor(ink).text(display(v),x+padding,y+padding,{width:cols[i]-padding*2,lineGap:2});x+=cols[i];});doc.strokeColor('#e4d9f4').moveTo(42,y+h).lineTo(553,y+h).stroke();y+=h;}
  row(headers,true);for(const r of rows)row(r);y+=16;
 }
 function shopping(rows){heading('Tools & materials');para('Check recorded ownership, quantities and condition before buying. Prices and store stock are not included.');table(['Item / specification','Quantity','Status'],rows.map(r=>[r.name+'\n'+(r.note||''),unitQuantity(r.qty,r.unit,units),r.owned?'Owned / allocated':'To source']),[323,85,103],{padding:6});}
 if(payload.input){
  ...
  page(title);para(dims,11,accent);if(units==='imperial')para(cutMeasurementNote(units,options.cutting),9);para('Use this revision throughout the build. Check already-cut pieces if your design has changed.',9);
  ...
  if(['cut','guide','drawings'].includes(payload.kind)){
   art(workshopArt(d,payload.kind==='guide'?'overview':'parts',{...options,units},'pdf'),payload.kind==='guide'?270:400);
   if(d.customShelves){page('Interior shelf depths');art(workshopArt(d,'shelf-depths',{...options,units},'pdf-depths'),360);para('Viewed from above. Dashed outlines show the full-depth footprint. Keep the back edges aligned.');}
   if(payload.kind==='guide'||d.customShelves)page('Cut list');
   ...
   table(['Part','Qty','Length','Width','Thickness'],d.parts.map(p=>[p.id+' · '+p.name,p.qty,exactCell(p.length),exactCell(p.width),exactCell(p.thickness)]),[219,45,82,82,83]);
  }
  ...
  if(['guide','drawings'].includes(payload.kind))for(const [i,s] of steps.entries()){
   ...
   if(s.id==='back'){page('Back fixing coordinates');para('Measure across and up from the outside bottom-left corner of the back. Each location meets a side, top, bottom or interior shelf edge. Use the back diagram to check orientation.');const rows=[],groups=Math.ceil(d.backScrews.length/3);for(let row=0;row<groups;row++){const values=[];for(let col=0;col<3;col++){const index=row+col*groups,p=d.backScrews[index];values.push(...(p?[index+1,exactCell(p.x),exactCell(p.y)]:['','','']));}rows.push(values);}table(['No.','Across','Up','No.','Across','Up','No.','Across','Up'],rows,[31,70,69,31,70,69,31,70,70],{padding:4,size:8.5});}
  }
