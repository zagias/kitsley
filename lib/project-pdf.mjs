...
export async function projectPDF(payload){
 ...
 function para(value,size=10.5,color=ink){const valueText=text(value);doc.font('Helvetica').fontSize(size);const h=doc.heightOfString(valueText,{width,lineGap:3});room(h+10);doc.fillColor(color).text(valueText,42,y,{width,lineGap:3});y+=h+10;}
 ...
 function table(headers,rows,cols){
  function row(values,isHead=false){doc.font(isHead?'Helvetica-Bold':'Helvetica').fontSize(9);let h=Math.max(...values.map((v,i)=>doc.heightOfString(text(v),{width:cols[i]-16,lineGap:2})))+18;if(y+h>bottom){page();row(headers,true);}if(isHead)doc.rect(42,y,width,h).fill('#f1e9fc');let x=42;values.forEach((v,i)=>{doc.fillColor(ink).text(text(v),x+8,y+8,{width:cols[i]-16,lineGap:2});x+=cols[i];});doc.strokeColor('#e4d9f4').moveTo(42,y+h).lineTo(553,y+h).stroke();y+=h;}
  row(headers,true);for(const r of rows)row(r);y+=16;
 }
 function shopping(rows){heading('Tools & materials');para('Check recorded ownership, quantities and condition before buying. Prices and store stock are not included.');table(['Item / specification','Quantity','Status'],rows.map(r=>[r.name+'\n'+(r.note||''),r.qty+' '+r.unit,r.owned?'Owned / allocated':'To source']),[323,85,103]);}
 if(payload.input){
  const d=workshopModel(payload.input),options=workshopOptions(payload.options),steps=workshopSteps(d,options);
  const dims=`${d.width} W × ${d.height} H × ${d.depth} D mm | ${d.thickness} mm ${d.materialName} | ${d.shelves} shelves`;
  ...
  page(title);para(dims,11,accent);para('Use this revision throughout the build. Check already-cut pieces if your design has changed.',9);
  ...
  if(['cut','guide','drawings'].includes(payload.kind)){
   art(workshopArt(d,payload.kind==='guide'?'overview':'parts',options,'pdf'),270);
   if(payload.kind==='guide')page('Cut list');
   para(d.melamine?`Core cut sizes in millimetres for ${d.materialName}. Add ${d.edgeThickness} mm front edging on A/B/C only; D remains 6 mm plywood. Do not subtract saw kerf. Request matching offcuts for a joint test.`:'Finished sizes in millimetres. For plywood, grain runs along length. D remains 6 mm plywood. Do not subtract saw kerf. Request matching offcuts for a joint test.',10);
   table(['Part','Qty','Length','Width','Thickness'],d.parts.map(p=>[p.id+' · '+p.name,p.qty,p.length,p.width,p.thickness]),[219,45,82,82,83]);
  }
  ...
  if(['guide','drawings'].includes(payload.kind))for(const [i,s] of steps.entries()){
   page(`${String(i+1).padStart(2,'0')}  ${s.title}`);para(dims,9,accent);para(s.parts,10);art(workshopArt(d,s.visual,options,'pdf-'+i),payload.kind==='drawings'?390:265);para(s.spec,11,accent);
   ...
   if(s.id==='back'){para('Back screw centres (across, up), mm from bottom-left:',9);para(d.backScrews.map(p=>`(${+p.x.toFixed(1)}, ${+p.y.toFixed(1)})`).join(' · '),9);}
  }
