...
export async function projectPDF(payload){
 ...
 if(payload.input){
  ...
  const dims=`${d.width} W × ${d.height} H × ${d.depth} D mm | ${d.thickness} mm plywood | ${d.shelves} shelves`;
  ...
  if(['cut','guide','drawings'].includes(payload.kind)){
   ...
   para('Finished sizes in millimetres. Grain runs along length. Do not subtract saw kerf. Request matching offcuts for a joint test.',10);
