import {safetyDecision} from './safety-policy.mjs';
import {domainsFor,diyDomains} from './diy-domains.mjs';
import {foundationAnswer,foundationVersion} from './diy-knowledge.mjs';
import {manualFor,manualVersion,isPaintManual} from './guide-manuals.mjs';
import {bookcaseAIContext} from './bookcase-ai-context.mjs';
export const knowledgeVersion='kitsley-guide-knowledge-2026-09-26.2';
const normalize=s=>String(s||'').toLowerCase().replace(/[?!.]+$/g,'').replace(/\s+/g,' ').trim();
function fingerprint(record,question,owned=[],companion={}){
 ...
 return JSON.stringify([knowledgeVersion,foundationVersion,companion,record?.id,record?.useCase,record?.projectPhase,record?.safetyFacts,manualVersion,record?.guideId,record?.manualOptions,record?.instructionProgress?.active,record?.setup,record?.pack?.input,record?.pack?.build?.key,record?.pack?.build?.active,record?.request,record?.notes,record?.answers,record?.stepAdvice,[...owned].sort(),history]);
}
export function internalAnswer(record,question,owned=[],companion={}){
 if(safetyDecision(record,question).profile)return null;
 const general=foundationAnswer(question);if(general)return general;
 if(!record?.guideId)return null;
 const q=normalize(question);if(!q||q.length>200)return null;
 const prior=(record.knowledge?.answers||[]).find(a=>a.question===q&&a.context===fingerprint(record,q,owned,companion)&&a.version===knowledgeVersion);
...
export function rememberAnswer(record,question,result,owned=[],companion={}){
 const source=result.source||'ai',q=normalize(question),previous=record.knowledge||{},answers=[...(previous.answers||[])];
 if(source==='ai'&&!result.designProposal&&!result.research&&!result.discovery&&result.text&&q.length>=12&&q.length<=200){const context=fingerprint(record,q,owned,companion);if(context.length>16000)return {version:1,answers:answers.slice(0,10),usage:previous.usage||{}};const existing=answers.findIndex(a=>a.question===q&&a.context===context);if(existing>=0)answers.splice(existing,1);answers.unshift({question:q,text:String(result.text).slice(0,3000),context,version:knowledgeVersion});}
 const topic=questionTopic(question),topics={...(previous.topics||{})};if(topic)topics[topic]=(topics[topic]||0)+1;
 const domains={...(previous.domains||{})};for(const id of new Set([...domainsFor(question,record).map(d=>d.id),...(result.specialistReview?.reviews||[]).map(r=>r.area)].filter(id=>diyDomains.some(d=>d.id===id))))domains[id]=Math.min(10000,(Number.isSafeInteger(domains[id])?domains[id]:0)+1);
 return {version:1,guideId:record.guideId,topics,domains,answers:answers.slice(0,10),usage:{guide:(previous.usage?.guide||0)+(source==='guide'?1:0),reused:(previous.usage?.reused||0)+(source==='saved-answer'?1:0),foundation:(previous.usage?.foundation||0)+(source==='foundation'?1:0),ai:(previous.usage?.ai||0)+(source==='ai'?1:0)},updatedAt:new Date().toISOString()};
}
