...
export function rememberAnswer(record,question,result,owned=[],companion={}){
 ...
 return {version:1,guideId:record.guideId,topics,answers:answers.slice(0,10),usage:{guide:(previous.usage?.guide||0)+(source==='guide'?1:0),reused:(previous.usage?.reused||0)+(source==='saved-answer'?1:0),foundation:(previous.usage?.foundation||0)+(source==='foundation'?1:0),ai:(previous.usage?.ai||0)+(source==='ai'?1:0)},updatedAt:new Date().toISOString()};
}
