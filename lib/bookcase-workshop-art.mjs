...
export function workshopArt(d,kind='overview',options={},namespace='bookcase'){
 ...
 if(kind==='overview'){
  art=cabinet(d,{paint:options.finish==='paint'});
  ...
  title='Labelled plywood parts and their exact sizes';
  ...
  art+=text(116,318,`${d.height} × ${d.panelDepth}`,14,'middle')+text(315,318,`${d.width-2*d.thickness} × ${d.panelDepth}`,14,'middle')+text(514,318,`${d.height} × ${d.width}`,14,'middle')+text(65,361,`A / B / C: ${d.thickness} mm plywood`)+text(65,388,'D: 6 mm plywood')+text(65,437,'Grain runs along the first dimension. All sizes in mm.',15);
 }else if(kind==='sand'){
  title='Hand sanding with the grain and smoothing the exposed plywood edge';
  art=panel(75,115,450,150,'#eddec9');for(let i=0;i<5;i++)art+=line(90,140+i*23,510,140+i*23,'#c6b497');
  ...
  art+=panel(75,265,450,18,'#c9b093');for(let i=1;i<4;i++)art+=line(76,265+i*4,524,265+i*4,'#eadfcf');art+=text(90,346,'220 grit on the sharp front edge',20)+line(120,321,120,282,violet)+text(90,402,'Keep veneer intact. Leave joint edges square.',16);
  ...
  for(const offset of d.holeOffsets){const y=340-offset/d.panelDepth*260;art+=`<ellipse cx="122" cy="${y}" rx="18" ry="7" fill="#cbb2f5" stroke="${ink}"/><ellipse cx="458" cy="${y}" rx="18" ry="7" fill="#cbb2f5" stroke="${ink}"/>`+line(490,y,519,y,'#aa95c4',true)+text(526,y+5,`${fmt(offset)} mm`,14);}
  ...
  title=options.finish==='paint'?'Painting the assembled bookcase with a brush':'Applying a clear finish to the bookcase';art=cabinet(d,{paint:options.finish==='paint'});
