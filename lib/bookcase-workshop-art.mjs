...
function cabinet(d,{flat=false,first=false,explode=false,paint=false}={}){
 ...
 for(const v of levels)out+=box(p,t,v.y,v.inset,w-2*t,t,v.depth,paint?'#efe9fa':'#ead9bf');
...
export function workshopArt(d,kind='overview',options={},namespace='bookcase'){
 ...
 if(kind==='overview'){
  ...
  art+=text(116,318,`${measure(d.height)} × ${measure(d.panelDepth)}`,units==='imperial'?11:14,'middle')+text(315,318,`${measure(d.width-2*d.thickness)} × ${measure(d.panelDepth)}`,units==='imperial'?11:14,'middle')+text(514,318,`${measure(d.height)} × ${measure(d.width)}`,units==='imperial'?11:14,'middle')+text(65,351,`A / B / C: ${measure(d.thickness)} ${d.materialName}`,d.melamine?13:15)+text(65,383,`D: ${measure(6)} plywood`)+text(65,420,d.melamine?`Core sizes exclude ${measure(d.edgeThickness)} front edging.`:'Plywood grain follows the first dimension.',14)+text(65,443,`D remains plywood. Sizes in ${units==='imperial'?'inches':'mm'}.`,14);
  if(d.customShelves){
   // Each row uses the same scale. No equal-sized shelf icons for unequal parts.
   art=text(35,34,'CUT PIECES · length × depth × thickness',18);
   const scale=Math.min(240/d.height,205/d.width),rowHeight=55;
   d.parts.forEach((p,i)=>{const y=52+i*rowHeight;art+=panel(42,y,p.length*scale,p.width*scale*.30,p.id==='D'?'#f0e7d8':'#e8ddfa')+text(306,y+14,`${p.id} · ${p.name} × ${p.qty}`,14)+text(306,y+34,[p.length,p.width,p.thickness].map(measure).join(' × '),13);});
   art+=text(35,416,`C1 is lowest; count upward. A/B/C: ${measure(d.thickness)} ${d.materialName}`,12)+text(35,439,d.melamine?`Core sizes exclude ${measure(d.edgeThickness)} front edging. D: plywood.`:'D: 6 mm plywood. Keep every back edge aligned.',13);
  }
