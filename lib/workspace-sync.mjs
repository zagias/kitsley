...
export const collections={
 ...
 'kitsley-owned':'tool', 'kitsley-sheet-stock':'stock', 'kitsley-equipment-v1':'equipment', 'kitsley-experience-v1':'experience'
};
const canonical=v=>Array.isArray(v)?v.map(canonical):v&&typeof v==='object'?Object.fromEntries(Object.keys(v).sort().map(k=>[k,canonical(v[k])])):v;
export const equal=(a,b)=>JSON.stringify(canonical(a))===JSON.stringify(canonical(b));
export const workspaceSchemaVersion=2;
// Older open tabs cannot represent new collections. Preserve those values instead
// of treating their absent fields/tombstones as an intentional deletion.
export function compatibleWorkspace(incoming,current,version=0){
 const next={...incoming};
 if(version<2)for(const [key,value] of Object.entries(current))if(/^(equipment|experience):/.test(key)&&next[key]==null)next[key]=value;
 return next;
}
export function flatten(storage){
 ...
 for(let i=0;i<storage.length;i++){const key=storage.key(i);if(/^kitsley-plan-/.test(key))out['draft:'+key.slice(13)]=JSON.parse(storage.getItem(key));if(/^kitsley-offer-choice-/.test(key)||key==='kitsley-interest-id'||key==='kitsley-units')out['preference:'+key]=storage.getItem(key);}
 return out;
}
...
export function validateEntities(value){
 ...
 for(const [key,v] of Object.entries(value)){
  if(!/^(conversation|saved|tool|stock|draft|preference|recovery|equipment|experience):[a-zA-Z0-9_-]{1,160}$/.test(key))throw Error('Invalid workspace item.');
  if(v===null)continue;
  const type=key.split(':')[0],id=key.slice(type.length+1);
  if(type==='equipment'&&(!validEquipment(v)||v.id!==id))throw Error('Invalid equipment details.');
  if(type==='experience'&&(!validExperience(v)||v.id!==id))throw Error('Invalid experience profile.');
