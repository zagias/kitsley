...
export function bookcaseAIContext(record){
 ...
 return {version:workshopVersion,scope:workshopScope,material:{name:d.materialName,core:d.material,frontEdgingMm:d.edgeThickness,back:"6 mm plywood",decisionAnswers:record.pack.input.materialNeeds||{}},dimensions:{width:d.width,height:d.height,depth:d.depth,thickness:d.thickness,shelves:d.shelves},interiorShelves:d.shelfPanels.map(p=>({part:p.id,position:p.name,coreDepth:p.width,finishedDepth:p.finishedDepth,frontSetback:p.inset,alignment:'back',pocketCentres:p.holeOffsets})),options,currentStep:progress.active+1,parts:d.parts,steps:steps.map(({id,title,parts,tools,spec,actions,check,detail})=>({id,title,parts,tools,spec,actions,check,detail})),supplies:workshopSupplies(d,options),sources:workshopSources};
}
...
export function conversationMessages(record,messages,owned=[]){
 ...
 const prefix=[{role:'user',content:(String(record.request||'')+(owned.length?'\nTools I have recorded in my kit: '+products.filter(p=>owned.includes(p.id)).map(p=>p.name).join(', '):'')).slice(0,2000)}];
