...
export function bookcaseAIContext(record){
 ...
 return {version:workshopVersion,scope:workshopScope,dimensions:{width:d.width,height:d.height,depth:d.depth,thickness:d.thickness,shelves:d.shelves},options,currentStep:progress.active+1,parts:d.parts,steps:steps.map(({id,title,parts,tools,spec,actions,check,detail})=>({id,title,parts,tools,spec,actions,check,detail})),supplies:workshopSupplies(d,options),sources:workshopSources};
}

export const bookcaseHelpInstructions=`This is help with an existing illustrated bookcase, not project discovery. Answer the user's question directly in up to 100 words. Do not ask a follow-up unless an essential fact is missing; no preference questions after a complete answer. Refer to the relevant numbered step when useful.
...
export function conversationMessages(record,messages){
 const workshop=record?.guideId==='bookcase'&&record?.pack?.build?.version===1;
 const prefix=[{role:'user',content:String(record.request||'').slice(0,2000)}];
