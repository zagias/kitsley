export function millimetres(value,unit='mm'){
 const factors={mm:1,cm:10,m:1000,in:25.4,inch:25.4,inches:25.4,'"':25.4};
 const n=Number(value)*factors[unit.toLowerCase()];
 if(!Number.isFinite(n)||n<=0)throw Error('Use a positive measurement with mm, cm, m or inches.');
 return Math.round(n);
}
export function parseDimensions(text){
 const match=text.trim().match(/^(\d+(?:\.\d+)?)\s*[x×,]\s*(\d+(?:\.\d+)?)\s*[x×,]\s*(\d+(?:\.\d+)?)\s*(mm|cm|m|inches|inch|in|")?$/i);
