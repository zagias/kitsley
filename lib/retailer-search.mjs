...
function sourceKey(value){const safe=publicUrl(value);if(!safe)return '';const u=new URL(safe);u.hash='';for(const key of [...u.searchParams.keys()])if(/^utm_|^(gclid|fbclid)$/i.test(key))u.searchParams.delete(key);return u.href.replace(/\/$/,'');}
export function suitableOffer(o,request){
 const specification=[o.product,o.specification].filter(Boolean).join(' ').replaceAll('1¼','1 1/4');
 if(/incompatible|not (?:a |an )?(?:match|compatible|suitable)|(?:thread|diameter|length|material|head type).{0,35}(?:differs|different|mismatch)/i.test(o.match||''))return false;
 // This fastener is a joint component, not an interchangeable general wood screw.
 if(/pocket[\s-]*(?:hole[\s-]*)?screws?/i.test(request.query)&&/coarse/i.test(request.query)&&/1¼|1[\s-]?1\/4|32\s*mm/i.test(request.query)){
  if(!/pocket[\s-]*(?:hole[\s-]*)?screws?/i.test(specification)||!/coarse/i.test(specification)||!/1[\s-]?1\/4|1\.25|32\s*mm/i.test(specification))return false;
  const unit=[o.unit,o.product].filter(Boolean).join(' '),pack=unit.match(/pack\s+of\s+(\d[\d,]*)|(\d[\d,]*)[\s-]*(?:pack|ct|count)/i);
  const count=pack?Number((pack[1]||pack[2]).replaceAll(',','')):null;
  if(count&&count>Math.max(100,request.quantity*10))return false;
 }
 return true;
}
export function normalizeResults(data,request){
 ...
 const sourceKeys=new Set([...sourceUrls].map(sourceKey));
 ...
 const offers=[],seen=new Set();let excludedCount=0;
 for(const o of (Array.isArray(json.offers)?json.offers:[]).slice(0,12)){
  const url=publicUrl(o.url);if(!url||!sourceKeys.has(sourceKey(url))||seen.has(url))continue;seen.add(url);
  if(!suitableOffer(o,request)){excludedCount++;continue;}
 ...
 return {offers,sources:[...sourceUrls].slice(0,10),excludedCount,candidateCount:Array.isArray(json.offers)?json.offers.length:0,checkedAt:new Date().toISOString(),query:request.query,quantity:request.quantity,location:request.location||'Selected location',radius:request.radius,mode:request.mode};
}
