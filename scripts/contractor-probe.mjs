...
for(const [domain,question,expectation] of scenarios.slice(start,start+count)){
 const record={request:question,safetyFacts:{location:'Ontario, Canada'}},local=directSafetyResponse(question)||compatibilityReply(question);
 try{
  if(local){console.log(JSON.stringify({domain,question,expectation,mode:'deterministic-'+local.source+'-response',answer:local}));continue;}
  const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal:AbortSignal.timeout(90000),headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL,store:false,max_output_tokens:4000,tools:[{type:'web_search',search_context_size:'medium'}],tool_choice:'required',include:['web_search_call.action.sources'],text:{format:researchFormat(null,{assessmentRequired:advicePurpose(record,question).assessmentRequired})},instructions:companionInstructions+' '+researchInstructions+' PROJECT ENGINE: '+JSON.stringify(engineContext(record,question))+' FOUNDATION: '+JSON.stringify(foundationContext(question,record))+' You are Kitsley, a concise DIY project assistant. Answer the actual question with one useful next action. Ask at most one consequential question, not something already answered. Text at most 150 words. Do not invent specifications, claim to update drawings or approve a safe load. No invasive electrical, gas, structural or hazardous-material procedures.',input:[{role:'user',content:question}]})});
