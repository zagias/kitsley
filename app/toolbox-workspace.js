...
export default function ToolboxWorkspace({owned,toggleOwned,addStarter,query,onQuery}){
 ...
 return <section className="inner-page compact-toolbox">
  <div className="toolbox-heading"><div><h1>My kit</h1><p>Your tools, equipment and materials. We’ll check what you have for every project.</p></div><button className="primary" onClick={()=>switchMode('add')}>＋ Add items</button></div>
  <div className="toolbox-switch" aria-label="Kit views">{[['owned','My items'],['add','Add items'],['materials','Materials']].map(([id,label])=><button key={id} aria-pressed={currentMode===id} onClick={()=>switchMode(id)}>{label}{id==='owned'&&owned.length>0?<span>{owned.length}</span>:null}</button>)}</div>
  {currentMode==='materials'?<><p className="toolbox-hint">Keep useful sheets and offcuts for your next build.</p><MaterialStock expanded/></>:<>
   ...
   {currentMode==='add'&&!query&&!category?<>
    ...
    <div className="toolbox-list-heading">{category?<button onClick={()=>{setCategory('');setPage(0);}}>← All categories</button>:<h2>{currentMode==='owned'?'What you have':'Search results'}</h2>}<span role="status">{filtered.length} {filtered.length===1?'item':'items'}</span></div>
    {category&&<h2>{categoryName}</h2>}
    {filtered.length===0?<div className="toolbox-empty"><WorkspaceIcon name="tools"/><h2>{currentMode==='owned'?'Let’s build your kit.':'No items found.'}</h2><p>{currentMode==='owned'?'Add the tools and supplies you already own. We’ll help you avoid buying them twice.':'Try a shorter name, such as “drill” or “screws”.'}</p>{currentMode==='owned'&&<button className="primary" onClick={()=>switchMode('add')}>Add my first items →</button>}</div>:<div className="toolbox-items">{filtered.slice(safePage*8,safePage*8+8).map(p=><article className="toolbox-item-card" key={p.id}><button className={owned.includes(p.id)?'is-owned':''} aria-pressed={owned.includes(p.id)} aria-label={`${p.name}: ${owned.includes(p.id)?'owned, tap to remove':'add to my items'}`} onClick={()=>toggleOwned(p.id)}><WorkspaceIcon name={toolGroups.find(g=>g[0]===toolGroup(p))?.[2]}/><span>{p.name}</span><span className="toolbox-item-action">{owned.includes(p.id)?'✓ Owned':'+ Add'}</span></button>{owned.includes(p.id)&&<EquipmentDetails toolId={p.id} name={p.name}/>}</article>)}</div>}
  ...
  <p className="toolbox-footnote"><SyncStatus/> · Project tool lists update automatically.</p>
 </section>;
}
