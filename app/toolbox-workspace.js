...
export default function ToolboxWorkspace({owned,toggleOwned,addStarter,query,onQuery}){
 ...
 return <section className="inner-page compact-toolbox">
  ...
  <div className="toolbox-switch" aria-label="Toolbox views">{[['owned','My items'],['add','Add items'],['materials','Materials']].map(([id,label])=><button key={id} aria-pressed={currentMode===id} onClick={()=>switchMode(id)}>{label}{id==='owned'&&owned.length>0?<span>{owned.length}</span>:null}</button>)}</div>
