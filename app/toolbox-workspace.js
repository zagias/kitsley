'use client';
import {EquipmentDetails,ExperienceProfile} from './companion-profile';
...
export default function ToolboxWorkspace({owned,toggleOwned,addStarter,query,onQuery}){
 const [mode,setMode]=useState(typeof window!=='undefined'&&new URLSearchParams(window.location.search).get('section')==='experience'?'experience':query?'add':'owned'),[category,setCategory]=useState(''),[page,setPage]=useState(0);
 ...
 return <section className="inner-page compact-toolbox">
  ...
  <div className="toolbox-switch" aria-label="Toolbox views">{[['owned','My items'],['add','Add items'],['materials','Materials'],['experience','My experience']].map(([id,label])=><button key={id} aria-pressed={currentMode===id} onClick={()=>switchMode(id)}>{label}{id==='owned'&&owned.length>0?<span>{owned.length}</span>:null}</button>)}</div>
  {currentMode==='experience'?<ExperienceProfile expanded/>:currentMode==='materials'?<><p className="toolbox-hint">Keep useful sheets and offcuts for your next build.</p><MaterialStock expanded/></>:<>
