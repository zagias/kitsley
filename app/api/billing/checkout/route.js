...
export async function POST(request){
 ...
 try{
  ...
  const session=await embeddedSession(stripe,{mode:body.plan==='plus'?'subscription':'payment',customer:customer.id,'payment_method_types[0]':'card','line_items[0][price]':priceId,'line_items[0][quantity]':'1',...metadata,...(body.plan==='plus'?{'subscription_data[metadata][kitsley_user]':user.id}:{'payment_intent_data[metadata][kitsley_user]':user.id}),ui_mode:'embedded',redirect_on_completion:'never','automatic_tax[enabled]':process.env.STRIPE_AUTOMATIC_TAX==='true'?'true':'false','customer_update[address]':'auto'},slot.attempt);
