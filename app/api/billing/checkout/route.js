...
import {billingUser,billingAccountAllowed,billingDB,billingReply,checkoutReady,ownsProject,grantsFor} from '../../../../lib/billing.mjs';
...
export async function POST(request){
 ...
 try{
  ...
  const session=await stripe('checkout/sessions',{mode:body.plan==='plus'?'subscription':'payment',customer:customer.id,'payment_method_types[0]':'card','line_items[0][price]':priceId,'line_items[0][quantity]':'1',...metadata,...(body.plan==='plus'?{'subscription_data[metadata][kitsley_user]':user.id}:{'payment_intent_data[metadata][kitsley_user]':user.id}),success_url:site+'/offers?checkout=returned'+(body.projectId?'&project='+encodeURIComponent(body.projectId):''),cancel_url:site+'/offers'+(body.projectId?'?project='+encodeURIComponent(body.projectId):''),expires_at:String(Math.floor(Date.parse(slot.expires_at)/1000)-60),'automatic_tax[enabled]':process.env.STRIPE_AUTOMATIC_TAX==='true'?'true':'false','customer_update[address]':'auto'},'kitsley-checkout-'+slot.attempt);
  if(!session.url)return billingReply({error:'This checkout has finished. Refresh your plan status before trying again.'},409);
  return billingReply({url:session.url});
