...
export async function POST(request){
 ...
 let q,projectId;try{const raw=await request.text();if(raw.length>3000)throw Error('Search is too long.');const input=JSON.parse(raw);projectId=input.projectId;q=validateSearch(input);}catch(e){return Response.json({error:e.message||'Check your search details.'},{status:400});}
 const allowed=billingEnabled()||await mutateStore(s=>{const day=new Date().toISOString().slice(0,10);if(s.retailerBudget?.day!==day)s.retailerBudget={day,used:0};const limit=Math.min(50,Math.max(1,Number(process.env.KITSLEY_RETAILER_DAILY_LIMIT)||10));if(s.retailerBudget.used>=limit)return false;s.retailerBudget.used++;return true;});
 ...
 const ticket=await reserveUsage(projectId,'search');if(ticket.response)return ticket.response;
 try{
  ...
  const result=normalizeResults(data,q);await recordUsage(ticket,data.usage);return Response.json(result,{headers:{'Cache-Control':'no-store'}});
 }catch(e){await releaseUsage(ticket);return Response.json({error:e.name==='TimeoutError'?'The search took too long. Try a more specific product or one retailer.':'Retailer search could not finish. Try a specific brand or model in a moment.'},{status:502});}
}
