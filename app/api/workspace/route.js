...
import {validateEntities,compatibleWorkspace} from '../../../lib/workspace-sync.mjs';
...
export async function PUT(request){
 ...
 try{const {client,user}=await session();if(!user)return reply({error:'Sign in again to sync.'},401);
  ...
  if(body.schemaVersion!==2){
   const {data:current,error:readError}=await client.from('account_workspaces').select('revision,entities').eq('user_id',user.id).maybeSingle();if(readError)throw readError;
   if(current&&current.revision!==body.revision)return reply({error:'Workspace changed on another device. Retrying.',conflict:true},409);
   body.entities=validateEntities(compatibleWorkspace(body.entities,current?.entities||{},0));
  }
