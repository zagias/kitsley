...
export default function FoundationBrowser(){
 ...
 return <section className="foundation-browser" aria-label="DIY knowledge reference">
 <header><span className="page-kicker">KITSLEY KNOW-HOW</span><h2>Find the detail you need.</h2><p>Materials, tools, home maintenance and trade planning. Each reference shows where it applies.</p></header>
 <div className="foundation-controls"><label htmlFor={id}>Find a detail<input id={id} type="search" value={query} placeholder="Try drywall tape, MERV, hinges or MDF screws…" onChange={e=>{setQuery(e.target.value);setCategory('');}}/></label><label>Topic<select value={category} onChange={e=>setCategory(e.target.value)}><option value="">All topics</option>{foundationCategories.map(c=><option key={c}>{c}</option>)}</select></label></div>
