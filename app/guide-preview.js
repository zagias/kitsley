...
export default function GuidePreview({entry,owned,onToggleOwned,onStart,onBack,onSave,isSaved}){
 ...
 {manual?<><IllustratedGuide key={entry.id} id={entry.id} options={options} onOptionsChange={setOptions} stop={entry.stop}/>{!kit.blocked&&kit.items.length>0&&<details className="guide-manual-disclosure"><summary>Check my kit <span>{kit.covered} of {kit.items.length} owned</span></summary><PackKit items={kit.items} owned={owned} onToggleOwned={onToggleOwned} scope={kit.scope}/></details>}<ManualSources manual={manual} entry={entry}/></>:<p>{entry.observe}</p>}
 </section>;
}
