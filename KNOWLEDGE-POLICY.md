...
## Planned cross-channel project memory

The [Future Projects Agent specification](FUTURE-PROJECTS-AGENT.md) extends these rules to persistent research tasks and conversations across the app, WhatsApp, SMS and email. Private project facts, accepted decisions and observed outcomes remain separate from reviewed shared knowledge. Changed inputs invalidate dependent conclusions; new research creates candidates rather than publishing its own approval. Research-job scheduling, channel delivery and shared-publication controls are planned, not implemented by this policy.
